/*
 * @behavior The shop's rules, with no database and no network: what a cart must look like,
 *           whether a sale window is open, the prices the buyer agrees to, and how an order
 *           moves through payment, fulfilment, refund, and dispute. Every answer is computed
...
/*
 * @behavior Turn what the shop page sent into the one cart shape the rest of the shop trusts —
 *           trimmed variants, whole positive quantities, no repeats — sorted so the same cart
 *           always looks identical.
 * @param items — the cart as the browser sent it
 * @returns a frozen, sorted list of { skuKey, quantity }
 * @exceptions throws when the cart is empty, repeats a variant, or asks for zero, a fraction,
 *             or a nonsensical quantity
 */
...
/*
 * @behavior Work out what this cart costs and lock it in — which variant, at what unit price,
 *           how many, and the total — the prices the buyer agrees to when they press Pay.
 * @param cart — the normalized cart
 * @param catalog — the price list rows for the open sale window
 * @param drop — the open sale window
 * @param now — the moment the prices are locked at
...
export function snapshotQuote({ cart, catalog, drop, now = new Date() }) {
  ...
  // The shop sells in US dollars only. A sale-window row has no currency field, and a stray
  // value must never change what a buyer is charged; readiness refuses any other currency.
  const currency = "usd";
...
/*
 * @behavior Apply one verified payment event to an order, keeping a paid order paid: an event
 *           that tries to move it back to pending is refused.
...
export function applyPaymentEvent(order, event = {}) {
  ...
  if (currentPaymentState === "paid") {
    ...
    // Already paid: keep the original paidAt and status.
    return { ...current };
  }
  ...
  if (event.type === "payment_confirmed" || event.status === "paid") {
    ...
    return {
      ...
      // Provider facts go in the settlement snapshot: a top-level providerPaymentId would be
      // silently dropped the moment the order is saved.
      settlementSnapshot: {
        ...settlement,
...
/*
 * @behavior Move an order one step through fulfilment — prepare, mark ready, hand over, post,
 *           hold, or cancel — refusing steps that are not allowed from where it is now.
 * @param order — the order as it is now
 * @param action — the step being asked for, and why (for a hold)
...
export function applyFulfillmentAction(order, action = {}) {
  const current = asRecord(order);
  // fulfillmentMethod is how the order document names this; anything but "shipping" is a pickup.
  ...
  // The hold is one document field: why it was placed, when, and where the order returns to.
  let fulfillmentHold = {
    reason: null,
    placedAt: null,
    returnToState: null,
...
/*
 * @behavior Track money going back to the buyer: reserve it, settle it, or drop a reservation —
...
/*
 * @behavior Record a card dispute on an order: when the buyer's bank opens one, and how it ends.
 * @param order — the order as it is now
