...
| `POST` | `/:id/purchases` | Add an itemized event purchase. Only after the event is published. The requester may log their own event's purchases; `events.purchases.complete` may log purchases for any published event. |
| `PATCH` | `/:id/checklist/purchases` | Mark the organizer's purchase log complete before post-event review. Requires the event to be published. |
...
Canonical event requests use `DRAFT → PVP_REVIEW → AGENDA → FINANCE_REVIEW → MARKETING_QUEUED → SCHEDULED → AWAITING_REVIEW → REVIEWED → ARCHIVED`, with `REJECTED` terminal. Legacy lowercase statuses remain readable during migration, but new spec-form requests use the canonical states. P/VP advance, return, and reject actions require the leadership permission; the board records its meeting outcome before the request is sent to the Director of Finance; Finance completes room booking and then records the funding decision against the original request; PR completion moves an approved request to `SCHEDULED`; the PR director then publishes the event (`events.publication.manage`) — surfaced as a "Ready to publish" queue on the dashboard — and purchases can only be logged once the event is published. `POST /:id/reviews` walks a scheduled, past-dated request through `AWAITING_REVIEW` to `REVIEWED` in one call, so a request never rests in `AWAITING_REVIEW`; the dashboard's "Post-event review" queue is scheduled requests with a completed purchase checkpoint whose event date has passed.
